...
function woocabandon_create_plugin_database_table() {
	...
	`finalised_at` DATETIME null DEFAULT NULL, 
...
function wooc_abandon_init() {
	class WC_Abandon {
		...
		public function __construct() {
			...
			add_action('woocommerce_checkout_update_order_review', array($this, 'get_vital_info'));
			add_action( 'wp_ajax_nopriv_capture_woocabandon_email',
				array( $this, 'capture_woocabandon_email_callback' ) );
		...
		function abandon_url_handler( $query ) {
			if ( $query->is_main_query() ) {
				...
				if ( $cookie ) {
					...
					$results = $wpdb->get_results( $wpdb->prepare( "select * from {$wpdb->prefix}woocabandon_carts where cookie = %s",
						$cookie ) );
		...
		public function _encode_customer($postData){
			...
			$whitelist = ['billing_address_1',
				'billing_address_2',
				'billing_first_name',
				'billing_last_name', 'billing_company', 'billing_email', 'billing_phone', 'billing_country', 'billing_company', 'billing_city', 'billing_state', 'billing_postcode'];
		...
		public function get_vital_info( $status = null, $email = null, $order = null ) {
			...
			$sql = "insert into {$wpdb->prefix}woocabandon_carts (cookie, unique_key, cart_contents, created_at, modified_at, email, synced_at, finalised_at, user_info) VALUES (%s,%s, %s, %s, %s, %s, null, %s, %s) ON DUPLICATE KEY UPDATE cart_contents = %s, user_info = %s, modified_at = %s, synced_at=null";

			$json_contents = json_encode( $contents );

			$date = date( "Y-m-d H:i:s" );
			$sql  = $wpdb->prepare( $sql, $cookie, $cookie . time(), $json_contents, $date, $date, $email,
				( $order ? $date : null ), $user_info, $json_contents, $user_info, $date );
			$wpdb->query( $sql );
